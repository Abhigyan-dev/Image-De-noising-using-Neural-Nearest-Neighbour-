...
parser = argparse.ArgumentParser(
    description='N3Net for Poisson-Gaussian image denoising')
...
# Data preprocessing
# whether to clip noisy images
utils.add_commandline_flag(parser, "--clip", "--no_clip", True)
# whether to train on monochrome or images with color filter array
utils.add_commandline_flag(parser, "--bayer", "--no_bayer", False)
# whether to concatenate noise level function to input
utils.add_commandline_flag(parser, "--inputnoisemap",
                           "--no_inputnoisemap", True)

# DnCNN
# Specification of DnCNNs: features, depth, kernelsize, activation, batchnorm
utils.add_commandline_networkparams(parser, "dncnn", 64, 6, 3, "relu", True)
# output channels of intermediate DnCNNs
parser.add_argument("--nfeatures_interm", type=int, default=8)
# number of DnCNN networks
parser.add_argument("--ndncnn", type=int, default=3)

# Nonlocal block
# Specification of embedding CNNs: features, depth, kernelsize, activation, batchnorm
utils.add_commandline_networkparams(parser, "embedcnn", 64, 3, 3, "relu", True)
# output channels of embedding CNNs
parser.add_argument("--embedcnn.nplanes_out", type=int, default=8)
# number of neighborhood volumes
parser.add_argument("--nl_k", type=int, default=7)
...
# whether to have separate temperature CNN
utils.add_commandline_flag(
    parser, "--nl_temp.external_temp", "--nl_temp.no_external_temp", True)
parser.add_argument("--nl_temp.temp_bias", type=float,
                    default=0.1)  # constant bias of temperature
# whether to have batch norm layer after calculat of pairwise distances
utils.add_commandline_flag(
    parser, "--nl_temp.distance_bn", "--nl_temp.no_distance_bn", True)
# in case of separate temperature CNN: whether to average pool temperature of each patch or to take temperature of center pixel
utils.add_commandline_flag(parser, "--nl_temp.avgpool",
                           "--nl_temp.no_avgpool", default=True)
    ...
    with torch.no_grad():
        for testloader, testname in testloaders:
            ...
            for k, stat in stats.items():
                add_summary(experiment, summaries,
                            testname + "/" + k, stat.avg)
...
def evaluate(experiment):
    ...
    with open(eval_file, "wb") as f:
        pickle.dump(dict(summaries=experiment.summaries),
...
def evaluate_image(experiment):
    ...
    if image_path[-4:] == ".mat":
        ...
        noisy = torch.from_numpy(np.float32(
            np.array(h5file['Inoisy']).T)).contiguous()
        ...
        for yy in range(2):
            for xx in range(2):
                ...
                denoised.data[0, 0, yy::2,
                              xx::2] = denoised_chan.data[0, 0, :, :]
...
def evaluate_dnd(experiment):
    ...
    dnd_dataset.eval_dnd(experiment, mode, out_dir, True,
...
def main():
    ...
    if args.eval:
        ...
        set_args = argparse.Namespace(
            **{k: v for k, v in dict(vars(args)).items() if v is not None})
        my_experiment = load(args.evaldir, set_args,
