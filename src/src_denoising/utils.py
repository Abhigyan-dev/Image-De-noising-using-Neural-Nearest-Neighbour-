...
def add_commandline_networkparams(parser, name, features, depth, kernel, activation, bn):
    parser.add_argument("--{}.{}".format(name, "features"),
                        type=int, default=features)
    parser.add_argument("--{}.{}".format(name, "depth"),
                        type=int, default=depth)
    parser.add_argument("--{}.{}".format(name, "kernel"),
                        type=int, default=kernel)
...
def get_args(args, basedir, saveload=True, args_default=None):
    ...
    if args.resume:
        if not args.resumedir:
            ...
            all_arg_diff = functools.reduce(lambda s1, s2: s1 | s2, dict_diff(
                args_other_d, args_resume_d), set())-{"resume", "resumedir", "resume_for_train", "resumeepoch"}
            if len(all_arg_diff) > 0 and saveload:
                print("Arguments differ:")
                for arg in all_arg_diff:
                    print("{}: {} \t\t {}".format(
                        arg, args_other_d[arg] if arg in args_other_d else "-", args_resume_d[arg]))
                # exit()
...
def parameters_by_module(net, name=""):
    ...
    params = [{"params": p, "name": n, "module": modulenames[m]}
              for m in net.modules() for n, p in m._parameters.items() if p is not None]
    return params
