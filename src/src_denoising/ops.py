...
class IndexedMatmul1Efficient(torch.autograd.Function):
    ...
    def forward(ctx, x, y, I):
        ...
        e = x.shape[2]
        out = torch.tensor(
            np.zeros(b*m*o), dtype=torch.float).reshape(b, m, o).cuda()
    ...
    def backward(ctx, grad):
        ...
        o = I.shape[2]
        e = x.shape[2]
        grad_x = torch.tensor(np.zeros(x.numel()), dtype=torch.float).reshape(
            x.shape[0], x.shape[1], x.shape[2]).cuda()
        grad_y = torch.tensor(np.zeros(y.numel()), dtype=torch.float).reshape(
            y.shape[0], y.shape[1], y.shape[2]).cuda()
...
class IndexedMatmul2Efficient(torch.autograd.Function):
    ...
    def forward(ctx, x, y, I, chunk_size=64):
        ...
        for m_offset in range(0, m, chunk_size):
            ...
            If = I_chunk.view(b, 1, this_chunk_size, o).expand(
                b, k, this_chunk_size, o)
            y_full = torch.cuda.FloatTensor(b, k, this_chunk_size, n).fill_(0)
            y_full = y_full.scatter_add(
                source=y_chunk.permute(0, 3, 1, 2), index=If, dim=3)
            z_interm = torch.cat(
                [torch.matmul(y_full[:, i_k:i_k+1, :, :], x_interm) for i_k in range(k)], 1)
    ...
    def backward(ctx, grad):
        ...
        for m_offset in range(0, m, chunk_size):
            ...
            I_chunk = I[:, m_offset:m_offset+this_chunk_size, :]
            y_chunk = y[:, m_offset:m_offset+this_chunk_size, :, :]
            grad_chunk = grad[:, m_offset:m_offset +
                              this_chunk_size, :, :].permute(0, 3, 2, 1)
            ...
            for i_k in range(k):
                grad_x += torch.matmul(grad_chunk[:, i_k, :, :],
                                       y_full[:, i_k, :, :]).permute(0, 2, 1)

            del y_full
            grad_y_full = torch.cat(
                [torch.matmul(x_interm, grad_chunk[:, i_k:i_k+1, :, :]) for i_k in range(k)], 1)
            del grad_chunk
            grad_y_chunk = grad_y_full.gather(
                2, If.permute(0, 1, 3, 2)).permute(0, 3, 2, 1)
            del grad_y_full
...
def im2patch(x, patchsize, stride, padding=None, returnpadding=False):
    padtop, padbottom, padleft, padright = calc_padding(
        x, patchsize, stride, padding)
...
def patch2im(x_patch, patchsize, stride, padding):
    padtop, padbottom, padleft, padright = padding
    counts = pyinn.col2im(torch.ones_like(x_patch), [
                          patchsize]*2, [stride]*2, [0, 0])
